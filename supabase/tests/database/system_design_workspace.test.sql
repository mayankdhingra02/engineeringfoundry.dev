...
select is((select count(*)::integer from public.system_design_item_catalog), 205, 'catalog contains every published concept and problem');
select is((select count(*)::integer from public.system_design_item_catalog where item_type = 'concept'), 178, 'catalog contains published concepts');
