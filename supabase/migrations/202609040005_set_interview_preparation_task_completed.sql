...
create or replace function public.toggle_interview_preparation_task(
  target_task_id uuid
)
returns boolean
language plpgsql
...
as $$
begin
  raise exception 'Desired-state preparation task saving is required'
    using errcode = '0A000';
end;
$$;
...
revoke all on function public.toggle_interview_preparation_task(uuid)
  from public, anon, authenticated;
grant execute on function public.toggle_interview_preparation_task(uuid)
  to authenticated;
