...
export async function getSystemDesignWorkspaceState(applicationId?: string | null) {
  ...
  if (!actor) return { signedIn: false as const, progress: {}, attempts: [], applications: [], application: null };
...
export async function getSystemDesignItemState(itemId: string, itemType: "concept" | "design_problem") {
  const actor = await getAuthenticatedActor();
  if (!actor) return { signedIn: false as const, progress: null };
  ...
  return { signedIn: true as const, progress: data };
}
...
export async function getSystemDesignProblemAttempts(problemId: string) {
  const actor = await getAuthenticatedActor();
  if (!actor) return { signedIn: false as const, attempts: [], applications: [] };
  ...
  return { signedIn: true as const, attempts: attempts.data ?? [], applications: applications.data ?? [] };
}
