...
import type { PreparationContinuation } from "./continuation";
...
export async function getAccountPreparationContinuations(): Promise<{ authenticated: boolean; candidates: PreparationContinuation[]; weeklyActivityDays: number }> {
  const actor = await getAuthenticatedActor();
  if (!actor) return { authenticated: false, candidates: [], weeklyActivityDays: 0 };
  ...
  const [upcomingRoundResult, preferencesResult, dsaResult, systemProgressResult, attemptsResult, trackResult, behavioralResult] = await Promise.all([
    ...
    supabase.from("system_design_attempts").select("id,problem_id,title,status,updated_at").eq("user_id", user.id).order("updated_at", { ascending: false }).limit(100),
  ...
  if ([upcomingRoundResult, preferencesResult, dsaResult, systemProgressResult, attemptsResult, trackResult, behavioralResult].some((result) => result.error)) return { authenticated: true, candidates: [], weeklyActivityDays: 0 };
  ...
  if (draft) candidates.push({ track: "system-design", title: `Continue ${draft.title}`, href: `/system-design/problems/${draft.problem_id}/practice/${draft.id}`, context: "Saved design attempt in progress.", source: "account", kind: "in-progress", updatedAt: timestamp(draft.updated_at) });
  ...
  for (const item of trackResult.data ?? []) {
    const catalog = item.track === "ml-design" ? mlCatalog.get(item.item_id) : behavioralCatalog.get(item.item_id);
    if (!catalog) continue;
  ...
  return { authenticated: true, candidates, weeklyActivityDays: preparationActivityDaysThisWeek(activityTimestamps.map((updatedAt) => ({ updatedAt }))) };
}
