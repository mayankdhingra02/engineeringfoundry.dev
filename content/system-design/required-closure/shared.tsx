...
import { requiredClosureSources, requiredClosureSourcesReviewedAt } from "./sources";

export interface RequiredClosureLessonSpec {
  id: string;
  decision: string;
...
export function RequiredClosureLesson({ spec }: { spec: RequiredClosureLessonSpec }) {
  return <>
    ...
    <MermaidDiagram chart={spec.diagram.chart} title={spec.diagram.title} description={spec.diagram.description} />
    ...
    <FurtherReading items={requiredClosureSources[spec.id] ?? []} />
    <p>Sources reviewed {requiredClosureSourcesReviewedAt}. Validate product limits, legal obligations, and operational assumptions for the system you are designing.</p>
