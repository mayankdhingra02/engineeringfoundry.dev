...
Export version `1.3` contains stable top-level keys including `export_version`, `generated_at`, `account`, `applications`, `interview_rounds`, `interview_preparation`, `behavioral`, `dsa`, `system_design`, `calendar`, `mock_interviews`, and `interview_experiences`. It includes private notes and structured attempts because they belong to the user. It omits passwords, hashes, sessions, tokens, provider delivery identifiers/errors, service credentials, reminder claims, and global curriculum/question catalogs.
