...
`save_account_preparation_preferences_if_revision` derives ownership from `auth.uid()` and accepts either an explicit absent state or the exact `updated_at` loaded with the complete three-field settings snapshot. One returned revision means the desired role, focus, and DSA roadmap were saved together; zero rows means another settings or preparation write won and the stale form changes nothing. The settings form preserves its draft and focus on conflict, offers a non-destructive latest-version link, and distinguishes a saved earlier snapshot from edits made while that request was pending. Role values are `sde1`, `sde2`, `senior`, `staff`, and `unsure`; focus values are `dsa`, `system_design`, `behavioral`, `applications`, and `unsure`. The DSA roadmap retains its established `sde1`, `sde2`, and `sde3plus` taxonomy.

Apply `202609040014_save_account_preparation_preferences_if_revision.sql` before deploying the revision-aware settings form. Migration-first makes already-loaded callers of the legacy full-snapshot RPC fail safely with SQLSTATE `0A000`; application-first makes new saves fail safely while the new RPC is absent but leaves old clients able to overwrite a newer snapshot until the migration lands. A code rollback after migration is safe but degraded because the legacy save remains unavailable; retain the migration and roll forward. Other preparation writers continue to update only their owned columns, advance the shared row revision, and therefore either run after a settings save or make a stale settings snapshot conflict instead of being overwritten.

## Authentication lifecycle

Display-name, email-change, password-change, and account-deletion actions treat their runtime payloads as unknown and strictly parse exact singleton string fields before authentication, profile, Auth, or deletion work. Non-`FormData`, missing, duplicate, file-valued, and unknown fields fail with curated errors instead of being string-coerced; framework action metadata is ignored explicitly. An empty display name remains the intentional remove operation, while a nonempty value is bounded before the authenticated profile update. Email changes call Supabase Auth `updateUser({ email })`; Auth remains authoritative and production verification behavior depends on the hosted Auth email-change configuration. Password changes enforce the same 8–128-character, letter-and-number policy as signup and recovery, then verify the current password with the provider before calling `updateUser({ password })`.
...
Export version `1.5` contains stable top-level keys including `export_version`, `generated_at`, `account`, `applications`, `interview_rounds`, `interview_preparation`, `behavioral`, `dsa`, `system_design`, `preparation_activity`, `calendar`, `mock_interviews`, `interview_experiences`, and `feedback`. It includes private notes, structured attempts, and the account’s own feedback submissions because they belong to the user. `preparation_activity` contains only canonical ML Design/Behavioral identifiers, status, and timestamps—never answer or story text. It omits passwords, hashes, sessions, tokens, provider delivery identifiers/errors, service credentials, reminder claims, and global curriculum/question catalogs.
