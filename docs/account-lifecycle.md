...
Export version `1.4` contains stable top-level keys including `export_version`, `generated_at`, `account`, `applications`, `interview_rounds`, `interview_preparation`, `behavioral`, `dsa`, `system_design`, `preparation_activity`, `calendar`, `mock_interviews`, and `interview_experiences`. It includes private notes and structured attempts because they belong to the user. `preparation_activity` contains only canonical ML Design/Behavioral identifiers, status, and timestamps—never answer or story text. It omits passwords, hashes, sessions, tokens, provider delivery identifiers/errors, service credentials, reminder claims, and global curriculum/question catalogs.
