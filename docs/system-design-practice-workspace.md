...
Attempt summaries include the integer revision but never the private structured document. The delete action strictly parses its bound attempt, problem, revision, and empty action form before account work, accepts only a correlated one-row RPC result, and reports database errors or malformed results without claiming deletion. Foreign, missing, stale, and problem-mismatched targets all return the same zero-row outcome without mutation. The legacy delete RPC remains callable only to return stable SQLSTATE `0A000`; clients cannot delete the table directly.
...
Deploy `202609040010_delete_system_design_attempt_if_revision.sql` before the revision-aware attempt confirmation UI. Migration-first makes already-loaded attempt-delete clients fail without mutation with SQLSTATE `0A000` and `Revision-checked System Design attempt deletion is required`; application-first makes only the new delete action fail safely while the RPC is missing and leaves the old stale-delete path available until migration. A post-migration application rollback is safe but degraded because the legacy delete remains unavailable; retain the migration and roll forward.
