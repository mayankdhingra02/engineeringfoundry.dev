...
Run `npm run validate:impact-ledger` to validate committed real JSON records. It rejects templates as real records, impossible/negative metric values, unknown definition versions, missing evidence references, and testimonials without explicit consent metadata. It deliberately does not calculate totals or generate claims.
