...
Module examples are editable in React page memory. Those edits are never persisted, transmitted, added to analytics, or placed in the URL. Copy uses only the device clipboard; if clipboard access fails, the edited draft receives focus and is selected for manual copying. The required situation set includes a standard counter, low offer, no competing offer, truthful competing offer, level mismatch, component trade, deadline extension, remote/geography question, internal raise/promotion, and final-limit decision.
