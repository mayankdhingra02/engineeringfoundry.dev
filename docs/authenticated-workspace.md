...
Custom preparation-task completion is also desired state, never a flip-current command. The browser submits the exact owned round, task, and target boolean; the owner-derived RPC locks that exact task, returns one correlated row for both a changed state and an idempotent repeat, and leaves `updated_at` unchanged for a no-op. Deletion requires the exact displayed `updated_at` and uses the same owner/task lock, so completion and deletion cannot silently overwrite each other; a stale, missing, foreign, or round-mismatched delete returns the same zero-row conflict without mutation. The hydrated confirmation control preserves focus and the submitted draft on a conflict and offers a non-destructive latest-record link; rendered focus and assistive-technology behavior remain browser/manual validation. Legacy toggle and delete RPCs are retained only as stable `0A000` no-mutation fail-safes. Apply `202609040011_delete_interview_preparation_task_if_revision.sql` before its application code so already-loaded stale delete clients fail before mutation; an application rollback remains safe but degraded and should roll forward.
