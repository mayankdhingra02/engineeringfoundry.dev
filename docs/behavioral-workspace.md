...
Apply `supabase/migrations/202608140001_create_behavioral_workspace.sql`, then the forward integrity, preparation-state, `202608140005_complete_behavioral_phase3.sql`, and `202608140006_enforce_behavioral_relationships.sql` migrations after the application tracker migration. Apply `202609030005_save_behavioral_story_aggregate.sql` before deploying the aggregate-writing application: the migration revokes authenticated direct story `INSERT`/`UPDATE` and theme mutations, and makes the legacy split theme RPC fail with SQLSTATE `0A000` and `Atomic Behavioral story saving is required`. Deploying the application first leaves already-loaded old clients able to perform split story/theme writes until the migration lands. After the migration, already-loaded old clients fail safely instead of producing a torn aggregate. Rolling application code back after the migration is safe but degraded because the old split-write path remains unavailable; keep the migration and roll forward to the aggregate client. In local development:
