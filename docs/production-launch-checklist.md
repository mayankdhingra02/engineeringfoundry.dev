...
4. [ ] `supabase migration list --linked` — confirm all 53 migrations are recorded, ending at `202609050001_create_dsa_practice_attempts`
...
6. [ ] Confirm every owner-scoped table reports `rowsecurity = true`:
   ...
   order by relname;
   ```
7. [ ] Confirm the canonical catalogs seeded: 162 DSA questions, 188 System Design concepts, 27 design problems, 48 curated Behavioral questions
...
Apply `202609040009_delete_behavioral_records_if_revision.sql` before deploying revision-aware Behavioral story and answer deletion. Migration-first makes already-loaded direct deletes fail safely with `42501`; the owner-derived delete RPCs share the story and question mutation locks and require the exact displayed revision. Application-first makes new deletes fail safely while the RPCs are absent but leaves the old stale destructive path available until migration. Rolling application code back after migration is safe but degraded because direct story and answer deletes stay unavailable; keep the migration and roll forward.
...
Apply `202609040013_harden_interview_preparation_task_add.sql` before deploying the strict custom-task action. Migration-first serializes every caller per owner/round before enforcing the 12-task cap and assigning the next position. Application-first rejects malformed browser input but leaves already-loaded or direct authenticated callers on the old count-then-insert race until migration. The RPC signature is unchanged, so an application rollback remains compatible; keep the migration and roll forward.

Apply `202609040014_save_account_preparation_preferences_if_revision.sql` before deploying revision-aware preparation settings. Migration-first makes already-loaded callers of `save_account_preparation_preferences` fail safely with SQLSTATE `0A000` and `Revision-checked preparation preference saving is required`. Application-first protects only new clients and leaves the old stale full-snapshot path open until migration. A post-migration application rollback is safe but degraded because legacy settings saves remain unavailable; keep the migration and roll forward.
...
- [ ] On a disposable round with 11 custom preparation tasks, race two `add_interview_preparation_task` calls and confirm exactly one succeeds, the other fails with SQLSTATE `P0001` and `Custom task limit reached`, and the final 12 tasks have distinct positions 0 through 11. Confirm `anon` and direct inserts fail with `42501`, malformed titles fail with `23514`, and foreign and missing rounds both fail with `P0002`. These hosted task-add checks remain unchecked until owner-run evidence is recorded.
...
- [ ] Create one custom preparation task as User A and confirm direct table `UPDATE` and `DELETE` are denied. Call `set_interview_preparation_task_completed` twice concurrently with the same `target_completed=true`; confirm both results contain that exact task, round, application, and desired state, the task remains complete, and repeating `true` does not change its timestamp. Race desired completion against exact-revision deletion and confirm exactly one wins: completion preserves the desired state with a newer revision, while deletion removes only the displayed revision. Confirm User B receives the same zero-row delete result for User A's task as for a nonexistent task, stale and round-mismatched deletes preserve the task, and no refused call changes User A's task. Confirm `anon` cannot execute the desired-state, revision-delete, or retired task mutations. Confirm authenticated legacy `toggle_interview_preparation_task` and `delete_interview_preparation_task` calls fail without mutation with SQLSTATE `0A000` and, respectively, `Desired-state preparation task saving is required` and `Revision-checked preparation task deletion is required`. These hosted custom-task checks remain unchecked until owner-run evidence is recorded.
...
- [ ] Confirm `anon` cannot execute `delete_system_design_attempt_if_revision` or the retired `delete_system_design_attempt`. Race one User A attempt save against deletion from the same exact integer revision and confirm exactly one wins: a winning save retains the complete worksheet and advances its revision, while a winning delete removes only the original snapshot. Confirm stale and problem-mismatched deletes preserve the attempt, User B's foreign target matches a missing zero-row result, direct table deletion is denied, and the authenticated legacy delete fails without mutation with SQLSTATE `0A000` and `Revision-checked System Design attempt deletion is required`. These hosted checks remain unchecked until owner-run evidence is recorded.
- [ ] Confirm `anon` cannot execute `create_behavioral_story_with_themes`, `update_behavioral_story_with_themes_if_revision`, `duplicate_behavioral_story_with_themes`, or `delete_behavioral_story_if_revision`; authenticated User A can create one parent with its exact controlled theme set.
...
- [ ] Race one User A full story save against `delete_behavioral_story_if_revision` from the same revision and confirm exactly one wins. Confirm a stale delete preserves the newer parent, themes, question links, and answer provenance; foreign and missing deletes return indistinguishable zero rows. Confirm authenticated direct story `INSERT`/`UPDATE`/`DELETE` and theme `INSERT`/`UPDATE`/`DELETE` fail. Confirm the authenticated legacy `replace_behavioral_story_themes` call fails with SQLSTATE `0A000` and `Atomic Behavioral story saving is required`.
- [ ] Confirm `anon` cannot execute `create_behavioral_answer_aggregate`, `update_behavioral_answer_aggregate_if_revision`, or `delete_behavioral_answer_if_revision`. As User A, create one answer with a desired primary state, race two full edits from the same revision, and confirm exactly one complete content/primary snapshot wins while the stale call returns zero rows without mutation.
...
- [ ] Race one User A full answer save against `delete_behavioral_answer_if_revision` from the same revision and confirm exactly one wins. Confirm stale, question-mismatched, foreign, and missing deletes return zero rows without changing the answer or its primary state. Confirm authenticated direct answer `INSERT`/`UPDATE`/`DELETE` fail while owner-scoped `SELECT` remains available. Confirm the authenticated legacy `set_behavioral_primary_answer` call fails without mutation with SQLSTATE `0A000` and `Atomic Behavioral answer saving is required`.
- [ ] Confirm `anon` cannot execute `save_behavioral_custom_question_if_revision` or `delete_behavioral_custom_question_if_revision`. As User A, create one custom question with a caller-generated ID and explicit absent revision; confirm one correlated result and a repeated absent save returns zero without mutation.
- [ ] Race two complete User A custom-question snapshots from one revision and confirm exactly one coherent question, context, category, company, and private-note snapshot wins. Confirm stale, foreign, and missing saves and deletes return indistinguishable zero-row outcomes and preserve the current question and linked preparation.
- [ ] Confirm authenticated direct custom-question `INSERT`, `UPDATE`, and `DELETE` fail with SQLSTATE `42501`. Delete with the exact revision and confirm one row is returned and its private mappings, saved-question references, and answer preparation cascade.
...
- [ ] Confirm `anon` cannot execute `save_account_preparation_preferences_if_revision`. As User A, create from an explicit absent revision, confirm one returned `updated_at`, and confirm an absent replay changes nothing. Race two complete role/focus/DSA-roadmap snapshots from one revision and confirm exactly one coherent winner. Race one full settings snapshot with a distinct desired DSA-roadmap update and confirm the newer roadmap survives while role, focus, and unrelated preparation-planning fields remain coherent. Confirm User B cannot observe or mutate User A's row, and the authenticated legacy `save_account_preparation_preferences` call fails without mutation with SQLSTATE `0A000` and `Revision-checked preparation preference saving is required`. These hosted checks remain unchecked until owner-run evidence is recorded.
...
- [ ] Export contains User A's data only, has `export_version` `1.6`, includes User A's `preparation_activity`, private DSA practice attempts, and account-linked feedback only, and contains no credentials, tokens, provider delivery identifiers, or global catalogs
