...
export function Footer() {
  return (
    <footer className="site-footer">
      ...
      <div className="footer-base page-width"><span>© {new Date().getFullYear()} Engineering Foundry</span><span>Built for engineers, with engineers.</span>{siteConfig.contactEmail ? <a href={`mailto:${siteConfig.contactEmail}`}>{siteConfig.contactEmail} <ArrowUpRight size={13} /></a> : <Link href="/contact">Contact options <ArrowUpRight size={13} /></Link>}</div>
    </footer>
  );
}
