  ...
  return (
    <footer className="site-footer">
      <div className="footer-main page-width">
        ...
        <div className="footer-links">{footerGroups.map((group) => <div key={group.title}><h3>{group.title}</h3>{group.links.map(([label, href]) => href.startsWith("/") ? <Link key={href} href={href}>{label}</Link> : <a key={href} href={href}>{label}</a>)}</div>)}</div>
      </div>
