...
function isTrack(value: string | null): value is MockTrack {
  return tracks.includes(value as MockTrack);
}

function isMode(value: string | null): value is MockPracticeMode {
  return value === "solo" || value === "peer";
}
...
export function MockInterviewLab() {
  const searchParams = useSearchParams();
  const initialTrack = isTrack(searchParams.get("track")) ? searchParams.get("track") as MockTrack : "dsa";
  const initialMode = isMode(searchParams.get("mode")) ? searchParams.get("mode") as MockPracticeMode : "solo";
  const initialPlans = plansForMockTrack(initialTrack);
  const requestedSlug = searchParams.get("problem");
  const [selectedTrack, setSelectedTrack] = useState<MockTrack>(initialTrack);
  const [selectedSlug, setSelectedSlug] = useState(initialPlans.some((plan) => plan.slug === requestedSlug) ? requestedSlug! : initialPlans[0].slug);
  const [mode, setMode] = useState<MockPracticeMode>(initialMode);
  ...
  function syncUrl(nextTrack = selectedTrack, nextSlug = selectedSlug, nextMode = mode) {
    const params = new URLSearchParams();
    params.set("track", nextTrack);
    params.set("problem", nextSlug);
  ...
  function chooseTrack(nextTrack: MockTrack) {
    ...
    setSelectedSlug(nextPlan.slug);
    syncUrl(nextTrack, nextPlan.slug, mode);
  }

  function choosePlan(slug: string) {
    clearSession();
    setSelectedSlug(slug);
    syncUrl(selectedTrack, slug, mode);
  }

  function chooseMode(nextMode: MockPracticeMode) {
    clearSession();
    setMode(nextMode);
    syncUrl(selectedTrack, selectedSlug, nextMode);
  }
  ...
  return <>
    <PageHero eyebrow="Mock Interview Practice Lab" title="Run a realistic practice session—solo or with your own peer." description="Choose an original prompt, use a suggested structure and session timer, then reflect with a qualitative rubric. No account, matching service, or saved feedback required.">
      ...
      <span className="hero-inline-note"><ShieldCheck size={15} /> Browser-memory only</span>
    </PageHero>
    ...
    <section className="section" id="session-builder"><div className="page-width">
      ...
      <div className="mock-builder">
        <fieldset className="mock-track-picker"><legend>1. Track</legend>{tracks.map((item) => <button type="button" key={item} className={selectedTrack === item ? "selected" : ""} aria-pressed={selectedTrack === item} onClick={() => chooseTrack(item)} disabled={sessionActive}><span>{mockTrackLabels[item]}</span><small>{plansForMockTrack(item).length} prompts</small></button>)}</fieldset>
