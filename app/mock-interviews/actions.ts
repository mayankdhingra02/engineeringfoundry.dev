...
export async function saveMockInterviewReview(
  ...
  const result = await actor.supabase.rpc("save_mock_interview_review", {
    ...
    target_prompt_exposure: validated.promptExposure,
    target_timing_mode: validated.timingMode,
    target_hint_policy: validated.hintPolicy,
    target_assistance_state: validated.assistanceState,
    target_session_outcome: validated.sessionOutcome,
    target_session_issue: validated.sessionIssue || null,
