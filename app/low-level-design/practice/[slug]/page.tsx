...
import { parseLowLevelDesignPracticeContext, type LowLevelDesignPracticeSearchParams } from "@/lib/low-level-design/practice-url-state";
...
export default async function LowLevelDesignPracticePage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<LowLevelDesignPracticeSearchParams> }) { const [{ slug }, rawSearchParams] = await Promise.all([params, searchParams]); const problem = lowLevelDesignPracticeBySlug.get(slug); if (!problem || problem.status !== "published") notFound(); return <LowLevelDesignPracticeView problem={problem} context={parseLowLevelDesignPracticeContext(rawSearchParams)} />; }
