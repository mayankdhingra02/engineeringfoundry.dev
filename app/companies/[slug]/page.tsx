...
export default async function CompanyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (interviewGuide) {
    ...
    const result = supabase ? await supabase.from("interview_experiences").select("id,role_title,role_level,region,interview_date,summary,interview_experience_rounds(round_type,topic_labels)").eq("status", "approved").eq("publication_consent", true).ilike("company_name", company.name).order("interview_date", { ascending: false, nullsFirst: false }).limit(6) : { data: [] };
