...
export default async function CompanyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><AnalyticsEventOnMount event="company_page_viewed" properties={{ company_slug: company.slug, company_name: company.name }} />
    ...
    <section className="section"><div className="page-width"><div className="company-path-grid"><article><Quote size={20} /><span className="section-kicker">Interview experiences</span><h2>Community context, once moderated.</h2><p>No submissions are claimed yet. The existing architecture is ready for a future reviewed experience flow.</p><Link className="card-link" href={`/interview-experiences/${company.slug}`}>View {company.name} experience state<ArrowRight size={14} /></Link></article><article><Users size={20} /><span className="section-kicker">Referrals</span><h2>Referral request preparation.</h2><p>Use the public toolkit to draft a request locally. This does not indicate that any {company.name} employee participates or that a referral will occur.</p><Link className="card-link" href="/referrals?mode=request">Open the referral toolkit<ArrowRight size={14} /></Link></article></div></div></section>
