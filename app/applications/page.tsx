  ...
  return <div className="tracker-workspace"><div className="page-width tracker-shell">
    ...
    {applications.length ? <>
      ...
      <div className="tracker-table-wrap"><table className="tracker-table"><thead><tr><th>Company</th><th>Role</th><th>Progress</th><th>Next interview</th><th>Applied</th><th>Status</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{visible.map((application) => {
        ...
        return <tr key={application.id}><td><Link href={`/applications/${application.id}`}><strong>{application.company_name}</strong></Link></td><td>{application.role_title}<small>{application.role_level || "Level not added"}</small></td><td>{progress.label}</td><td>{next ? `${next.round_type} · ${formatInterviewDate(next.scheduled_at, next.timezone)}` : "Nothing scheduled"}</td><td>{formatApplicationDate(application.application_date)}</td><td><StatusForm id={`${application.id}-table`} status={application.status} action={statusAction} compact /></td><td><div className="tracker-row-actions"><Link href={`/applications/${application.id}`}>Open</Link><Link href={`/applications/${application.id}/edit`}>Edit</Link><ConfirmAction action={deleteAction} label="Delete" /></div></td></tr>;
      })}</tbody></table></div>
      <div className="tracker-mobile-list">{visible.map((application) => { const next = nextRound(application); const progress = roundProgress(application.interview_rounds); const statusAction = updateApplicationStatusAction.bind(null, application.id); const deleteAction = deleteApplicationAction.bind(null, application.id); return <article key={application.id}><div className="tracker-mobile-heading"><div><Link href={`/applications/${application.id}`}>{application.company_name}</Link><span>{application.role_title}{application.role_level ? ` · ${application.role_level}` : ""}</span></div><span className={`tracker-badge ${badge(application.status)}`}>{application.status}</span></div><dl><div><dt>Next interview</dt><dd>{next ? `${next.round_type} · ${formatInterviewDate(next.scheduled_at, next.timezone)}` : "Nothing scheduled"}</dd></div><div><dt>Progress</dt><dd>{progress.label}</dd></div><div><dt>Applied</dt><dd>{formatApplicationDate(application.application_date)}</dd></div></dl><StatusForm id={application.id} status={application.status} action={statusAction} compact /><div className="tracker-mobile-actions"><Link href={`/applications/${application.id}`}>Open</Link><Link href={`/applications/${application.id}/edit`}>Edit</Link><ConfirmAction action={deleteAction} label="Delete" /></div></article>; })}</div>
