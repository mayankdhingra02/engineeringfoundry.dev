...
export default async function ExperienceCompanyPage({ params }: { params: Promise<{ company: string }> }) {
  ...
  return <>
    ...
    <section className="section"><div className="page-width"><SectionHeading eyebrow="Current reviewed experiences" title={`Reviewed ${item.name} contributor reports.`} description="Only approved reports with publication consent appear here. No fabricated entries, counts, exact questions, or candidate identities are shown." /><ExperienceDirectory availability={publicResult.availability} experiences={experiences} /></div></section>
