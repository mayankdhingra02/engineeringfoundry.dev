...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const dynamic = [...companies.map((c) => `/companies/${c.slug}`), ...companies.map((c) => `/interview-experiences/${c.slug}`), ...dsaTopics.map((topic) => `/dsa/${topic.slug}`), ...activeSystemDesignProblems.map((problem) => `/system-design/${problem.slug}`), ...activeMlDesignProblems.map((problem) => `/ml-design/${problem.slug}`), ...activeChallenges.map((challenge) => `/challenges/${challenge.slug}`)];
