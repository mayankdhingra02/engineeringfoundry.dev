...
export default function sitemap(): MetadataRoute.Sitemap {
  const routes = ["", "/prepare", "/dsa", "/system-design/start-here/introduction", "/system-design/problems", "/system-design/plan", "/ml-design", "/behavioral", "/interview-tips", "/interview-tips/rounds", "/companies", "/resources", "/mock-interviews", "/referrals", "/interview-experiences", "/challenges", "/leaderboard", "/community", "/about", "/faq", "/contact", "/privacy", "/terms"];
  const dynamic = [...companies.map((c) => `/companies/${c.slug}`), ...companies.map((c) => `/interview-experiences/${c.slug}`), ...dsaTopics.map((topic) => `/dsa/${topic.slug}`), ...dsaCurriculumPages.map((page) => page.slug!), "/dsa/questions", "/dsa/roadmap", "/dsa/patterns", ...systemDesignLessons.filter((lesson) => lesson.status === "published").map((lesson) => lesson.slug!), ...activeMlDesignProblems.map((problem) => `/ml-design/${problem.slug}`), ...activeChallenges.map((challenge) => `/challenges/${challenge.slug}`), ...V1_ROUND_EXECUTION_GUIDES.map((guide) => roundExecutionGuideHref(guide.slug))];
