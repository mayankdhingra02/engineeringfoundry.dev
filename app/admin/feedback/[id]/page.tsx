...
export default async function AdminFeedbackDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await actor.supabase.from("feedback_submissions").select("id,reference_id,category,status,message,page_context,contact_email,contact_consent,submitted_as_authenticated,created_at,admin_note").eq("id", id).maybeSingle();
  if (!data) notFound();
