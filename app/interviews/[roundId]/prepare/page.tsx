...
export default async function InterviewPreparationPage({ params }: { params: Promise<{ roundId: string }> }) {
  ...
  return <div className="prep-hub" data-direction-seed="68d019a3"><div className="page-width prep-shell">
    ...
    <aside className="prep-side"><section className="prep-checklist"><header><ListChecks size={20} /><div><h2>Round checklist</h2><p>{hub.completedCount} of {hub.totalCount} complete</p></div></header><div className="prep-progress-track" aria-label={`${hub.completedCount} of ${hub.totalCount} preparation tasks complete`}><span style={{ width: `${hub.totalCount ? (hub.completedCount / hub.totalCount) * 100 : 0}%` }} /></div><ul>{checklist.map((item) => { const complete = completedIds.includes(item.id); const action = togglePreparationChecklistAction.bind(null, round.id, applicationId, completedIds, item.id); return <li key={item.id}><PreparationChecklistControl action={action} complete={complete} label={item.label} /></li>; })}{tasks.map((task) => { const toggle = togglePreparationTaskAction.bind(null, round.id, applicationId, task.id); const remove = deletePreparationTaskAction.bind(null, round.id, applicationId, task.id); return <li key={task.id} className="custom"><PreparationTaskControl action={toggle} complete={task.completed} label={task.title} /><PreparationTaskDeleteControl action={remove} /></li>; })}</ul>{tasks.length < 12 && <PreparationAddTaskForm action={addPreparationTaskAction.bind(null, round.id, applicationId)} />}</section>
