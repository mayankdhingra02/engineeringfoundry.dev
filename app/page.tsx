...
import { ArrowRight, Binary, BrainCircuit, CheckCircle2, Code2, FileEdit, GitPullRequestArrow, MessagesSquare, Network, ServerCog, ShieldCheck, Trophy, Users } from "lucide-react";
...
export default function HomePage() {
  return (
    <>
      ...
      <section className="section section-alt"><div className="page-width"><div className="home-experience-callout"><FileEdit size={24} /><div><span className="section-kicker">Interview experience write-up</span><h2>Document interview experiences safely.</h2><p>Create a structured, privacy-conscious reflection entirely in your browser. Nothing is saved or published; a moderated public directory is planned for a later authenticated phase.</p></div><Link className="button" href="/interview-experiences">Open the write-up builder <ArrowRight size={15} /></Link></div></div></section>
      ...
      <section className="section section-dark"><div className="page-width"><SectionHeading eyebrow="Engineering Challenge Lab" title="Practice engineering judgment, not just answers." description="Work through eight original, self-guided scenarios with concrete deliverables, revealable considerations, qualitative rubrics, and session-only worksheets." action={<Link className="button button-inverse" href="/challenges">Open the Challenge Lab <ArrowRight size={15} /></Link>} /><div className="challenge-track-grid">{challengeTracks.map(({ icon: Icon, label }, index) => <article key={label}><span>{String(index + 1).padStart(2, "0")}</span><Icon size={21} /><h3>{label}</h3><p>Original scenario · concrete deliverable · self-review</p></article>)}</div></div></section>

      <section className="section community-section"><div className="page-width"><div className="community-panel"><div><div className="eyebrow"><Users size={13} />1,000+ community members</div><h2>Don&apos;t prepare alone.</h2><p>Explore public learning, practice, and career pathways, then join the existing Discord community for optional discussion and peer connection. Membership is not an active-user metric.</p><div className="community-topics"><span>Interview discussions</span><span>Study partners</span><span>Mock partners</span><span>Challenge approaches</span><span>Career etiquette</span><span>Constructive feedback</span></div><div className="hero-actions"><TrackedLink className="button" href={siteConfig.discordUrl} event="discord_clicked" properties={{ placement: "homepage_community" }}>Join the Discord <ArrowRight size={15} /></TrackedLink><Link className="button button-secondary" href="/community">Open Community Hub</Link></div></div><div className="community-grid-visual" aria-hidden="true"><span className="node core">EF</span><span className="node n1">DSA</span><span className="node n2">SYSTEMS</span><span className="node n3">MOCKS</span><span className="node n4">CAREER</span><i className="line l1" /><i className="line l2" /><i className="line l3" /><i className="line l4" /></div></div></div></section>
