...
const preparationAreas = [
  ...
  { icon: Users, title: "Mock Interviews", description: "Practice interview conditions with other engineers and exchange useful feedback.", href: "/mock-interviews", cta: "Find a Mock Interview" },
  { icon: GitPullRequestArrow, title: "Referrals", description: "Connect with employees willing to independently consider referral requests.", href: "/referrals", cta: "Explore Referrals" },
];
...
export default function HomePage() {
  return (
    <>
      ...
      <section className="section"><div className="page-width"><SectionHeading eyebrow="Prepare by company" title="Understand the interview in context." description="Company guides will bring DSA, System Design, behavioral preparation, interview experiences, and community referral activity into one view." action={<Link className="button button-ghost" href="/companies">Browse Company Guides <ArrowRight size={15} /></Link>} /><div className="home-company-grid">{companies.map((company, index) => <Link className="home-company-card" href={`/companies/${company.slug}`} key={company.slug}><span className="company-seal">{company.name.slice(0, 2).toUpperCase()}</span><span><strong>{company.name}</strong><small>{company.focusAreas.slice(0, 3).join(" · ")}</small></span><ArrowRight size={15} aria-hidden="true" /><i>{String(index + 1).padStart(2, "0")}</i></Link>)}</div></div></section>

      <section className="section section-alt"><div className="page-width"><div className="home-split home-mock"><div><div className="eyebrow"><Users size={13} />Mock interviews</div><h2>Practice with other engineers before the real interview.</h2><p>Knowing the material is different from explaining it under pressure. Practice the format, communicate your reasoning, and exchange structured feedback with a peer.</p><div className="type-list" aria-label="Mock interview types">{interviewTypes.map((type) => <span key={type}><CheckCircle2 size={14} />{type}</span>)}</div><div className="hero-actions"><Link className="button" href="/mock-interviews">Find a Mock Interview <ArrowRight size={15} /></Link></div><p className="availability-note">Experienced interviewer sessions are planned for a later phase.</p></div><div className="mock-console" aria-label="Mock interview matching preview"><div className="console-label"><span>PEER MATCH / PREVIEW</span><span>01 — 03</span></div><div className="match-row"><span>Interview type</span><strong>System Design</strong></div><div className="match-row"><span>Experience level</span><strong>Experienced</strong></div><div className="match-row"><span>Practice focus</span><strong>Scoping + tradeoffs</strong></div><div className="match-state"><i /><span>Looking for a compatible peer</span></div></div></div></div></section>

      <section className="section"><div className="page-width"><div className="home-split referral-highlight"><div className="referral-visual"><div className="referral-flow">{["Find a company", "Send a request", "Referrer reviews", "Referrer decides"].map((step, index) => <div key={step}><span>{String(index + 1).padStart(2, "0")}</span><strong>{step}</strong>{index < 3 && <ArrowRight size={15} />}</div>)}</div><p><ShieldCheck size={15} /> A referral is never guaranteed. Referrers make their own decision and must follow employer policies.</p></div><div><div className="eyebrow"><GitPullRequestArrow size={13} />Community referrals</div><h2>Your network shouldn&apos;t end with who you already know.</h2><p>Engineers can volunteer as <strong>Referrers</strong> and independently review relevant requests. The process is transparent, voluntary, and designed around useful context—not transactions.</p><div className="hero-actions"><Link className="button" href="/referrals">Explore Referrals</Link><Link className="button button-secondary" href="/referrals">Become a Referrer</Link></div></div></div></div></section>
