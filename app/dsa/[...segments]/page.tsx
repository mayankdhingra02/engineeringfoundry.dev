...
import { dsaPatterns, questionsForPattern, questionsForTopic, roadmapStages, topicBySlug } from "@/data/dsa";
import { dsaPatternLessonBySlug } from "@/data/dsa/pattern-lessons";
...
import { PatternLesson } from "@/features/dsa/pattern-lesson";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (segments.length === 2 && segments[0] === "patterns") {
    const pattern = dsaPatternLessonBySlug.get(segments[1]);
    if (!pattern) notFound();
    return createPageMetadata({ title: `${pattern.name} Coding Interview Pattern`, description: pattern.problemShape, path });
  }
...
function PatternIndex() {
  ...
  return <DSAArticleLayout page={page} showPager={false}><p>Patterns help compress search space, but they are useful only when connected to recognition signals and an invariant. Use this index to name weak areas, study the full derivation, then practice without a visible label.</p><div className="pattern-grid">{dsaPatterns.map((pattern) => { const questionCount = questionsForPattern(pattern.slug).length; return <article key={pattern.id}><h2><Link href={`/dsa/patterns/${pattern.slug}`}>{pattern.name}</Link></h2><p>{pattern.summary}</p><h3>Recognition signals</h3><ul>{pattern.recognitionSignals.map((signal) => <li key={signal}>{signal}</li>)}</ul><h3>Common mistakes</h3><ul>{pattern.commonMistakes.map((mistake) => <li key={mistake}>{mistake}</li>)}</ul><div className="pattern-grid-actions"><Link className="card-link" href={`/dsa/patterns/${pattern.slug}`}>Study the full lesson<ArrowRight size={14} /></Link>{questionCount > 0 ? <Link className="card-link" href={`/dsa/questions?q=${encodeURIComponent(pattern.slug)}`}>Practice {questionCount} matching question{questionCount === 1 ? "" : "s"}<ArrowRight size={14} /></Link> : <span className="pattern-grid-empty">No matching questions cataloged yet.</span>}</div></article>; })}</div><DSAHeading level={2} id="use-patterns-well">Use patterns without memorizing answers</DSAHeading><p>For each attempt, record the clue that suggested a pattern, the invariant that makes it correct, and the condition that would make you choose something else.</p></DSAArticleLayout>;
}

function PatternDetailPage({ slug }: { slug: string }) {
  const lesson = dsaPatternLessonBySlug.get(slug); if (!lesson) notFound();
  const questions = questionsForPattern(slug);
  const workedProblem = questions.find((question) => question.id === lesson.workedProblemId);
  if (!workedProblem) throw new Error(`Missing worked problem ${lesson.workedProblemId} for ${slug}.`);
  return <PatternLesson lesson={lesson} questions={questions} workedProblem={workedProblem} />;
}
...
async function TopicPage({ slug }: { slug: string }) {
  ...
  const patterns = patternSlugs.map((patternSlug) => dsaPatternLessonBySlug.get(patternSlug)).filter((pattern): pattern is NonNullable<typeof pattern> => Boolean(pattern));
  ...
  const state = await getDsaWorkspaceState();
  return <TopicLesson topic={topic} questions={topicQuestions} patterns={patterns} stage={stage} nextStage={nextStage} progress={state.progress} signedIn={state.signedIn} accountPlatformAvailable={state.accountPlatformAvailable} />;
}
...
export default async function DsaCatchAllPage({ params, searchParams }: PageProps) {
  ...
  if (segments.length === 2 && segments[0] === "patterns") return <PatternDetailPage slug={segments[1]} />;
