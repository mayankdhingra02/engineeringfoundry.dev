...
export async function generateMetadata({ params }: { params: Promise<{ topic: string }> }): Promise<Metadata> { const { topic } = await params; const title = dsaTopics.slice(1).find((item) => item.toLowerCase().replaceAll(" ", "-") === topic); if (!title) notFound(); return createPageMetadata({ title: `${title} DSA`, description: `${title} interview preparation roadmap.`, path: `/dsa/${topic}` }); }
