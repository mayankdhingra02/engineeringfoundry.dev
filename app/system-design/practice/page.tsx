...
export default async function SystemDesignPracticePage({ searchParams }: { searchParams: Promise<{ application?: string; q?: string; kind?: string; status?: string; bookmarked?: string }> }) {
  ...
  if (!state.signedIn) return state.accountPlatformAvailable
    ? <div className="sd-doc-layout"><SystemDesignSidebar curriculum={systemDesignCurriculum} accountPlatformAvailable /><div className="sd-practice-home signed-out"><section><LockKeyhole size={24} /><h1>Your System Design practice stays private</h1><p>Public lessons and walkthroughs remain available without an account. Sign in to keep concept notes, confidence, bookmarks, and independent design attempts.</p><div><Link className="button" href="/signin?next=/system-design/practice">Sign in</Link><Link className="button button-secondary" href="/system-design/problems">Browse public problems</Link></div></section></div></div>
    : <div className="sd-doc-layout"><SystemDesignSidebar curriculum={systemDesignCurriculum} accountPlatformAvailable={false} /><div className="sd-practice-home signed-out"><section><BookOpen size={24} aria-hidden="true" /><h1>Public System Design practice remains available</h1><p>Account-backed practice is unavailable right now. Browse a problem for an unsaved rehearsal, or continue through the course without an account.</p><div><Link className="button" href="/system-design/problems">Browse public problems</Link><Link className="button button-secondary" href="/system-design/start-here/introduction">Start learning</Link></div></section></div></div>;
