...
export function OfferComparisonWorksheet() {
  ...
  return <section className="salary-worksheet" id="compare-offers" aria-labelledby="offer-comparison-heading">
    <header><div><h2 id="offer-comparison-heading">Private offer comparison</h2><p>Offer values, labels, notes, dates, and draft text stay only in this browser session and never leave it. Visiting this page sends one fixed, value-free analytics event; it never includes anything you enter. Closing this page clears the worksheet.</p></div><button className="button button-secondary button-sm" type="button" onClick={() => setOffers((current) => current.length < MAX_OFFERS ? [...current, emptyOffer(current.length + 1)] : current)} disabled={offers.length >= MAX_OFFERS}><Plus size={15} />Add offer</button></header>
