...
function ResourceGroup({ guide, category }: { guide: CompanyInterviewGuide; category: string }) {
  ...
  return <div className="company-resource-group"><h3>{category}</h3><div>{resources.map((resource) => {
    ...
    return href.startsWith("/") ? <Link href={href} key={resource.title}>{content}</Link> : <a href={href} target="_blank" rel="noopener noreferrer" key={resource.title}>{content}</a>;
  })}</div></div>;
}
    ...
    {!embedded && <header className="company-guide-hero">
      <div className="page-width company-guide-hero-inner">
        ...
        <div className="company-guide-hero-actions">
          ...
          <a className="button button-secondary company-official-link" href={guide.officialUrl} target="_blank" rel="noopener noreferrer">{guide.officialLinkLabel ?? "Official company guide"}<ExternalLink size={14} /></a>
      ...
      <div className="company-guide-main">
        ...
        <GuideSection id="full-guide" title="Full guide and research reference" description="Open only the deeper evidence, edge cases, and supporting material you need. Nothing has been removed.">
          <div className="company-reference-stack">
            ...
            <ReferencePanel id="resources" title="Resources" summary="Official material first, followed by clearly labeled practice resources"><div className="company-resource-groups">{guide.resourceCategories.map((category) => <ResourceGroup guide={guide} category={category} key={category} />)}</div>{guide.systemsReading && <div className="company-systems-reading">{guide.systemsReading.map((item) => <a href={item.url} target="_blank" rel="noopener noreferrer" key={item.title}><span><strong>{item.title}</strong><small>{item.lesson}</small></span><ExternalLink size={14} /></a>)}</div>}</ReferencePanel>
