...
export function PracticeWorkspace({ accountPlatformAvailable, signedIn, progress, preferredRoadmap, application, libraryOnly = false }: { accountPlatformAvailable: boolean; signedIn: boolean; progress: DsaProgressMap; preferredRoadmap: RoadmapLevel; application: PracticeApplication; libraryOnly?: boolean }) {
  ...
  const description = libraryOnly
    ? accountPlatformAvailable ? "Search public question metadata and, when signed in, update progress without leaving the library." : "Search public question metadata and open any question for browser-local practice. Account-backed progress is unavailable in this configuration."
    : accountPlatformAvailable ? "Resume the highest-priority work, review weak questions, and keep one account-backed record across the library and roadmaps." : "Browse public question metadata and record completion on individual question pages in this browser. Account-backed progress is unavailable in this configuration.";
  return <DSAWorkspacePageLayout eyebrow={libraryOnly ? "Question database" : undefined} title={title} description={description} badge={signedIn ? "Private progress · public questions" : "Public metadata"} meta={`${dsaInterviewQuestionDatabase.length} questions · ${representedCompanies} companies represented`}>
    ...
    {!signedIn ? accountPlatformAvailable ? <aside className="dsa-practice-signin"><ShieldCheck size={20} /><div><strong>Keep progress between sessions</strong><p>The public library stays open. Sign in to save statuses, confidence, bookmarks, and private notes.</p></div><Link className="button button-secondary button-sm" href={`/signin?next=${encodeURIComponent(libraryOnly ? "/dsa/questions" : "/dsa/practice")}`}>Sign in</Link></aside> : <aside className="dsa-practice-signin"><ShieldCheck size={20} aria-hidden="true" /><div><strong>Public practice remains available</strong><p>Open a question to record completion in this browser. Private notes and account-backed progress are unavailable in this configuration.</p></div></aside> : !libraryOnly && <>
