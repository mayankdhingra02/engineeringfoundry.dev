...
export function PatternReference() {
  return <div className="dsa-strategy-patterns">{patternCues.map((cue) => {
    ...
    const topicHref = getCoreRoadmapTopicHref(topic.id);
    const practiceHref = getRoadmapPracticeHref(topic);
    return <article key={cue.signal}><span>{cue.signal}</span><strong>{cue.pattern}</strong><nav aria-label={`${cue.pattern} resources`}>{topicHref && <Link href={topicHref}><Route size={12} />Roadmap<ArrowRight size={11} /></Link>}{practiceHref && <Link href={practiceHref}>Practice<ArrowRight size={11} /></Link>}</nav></article>;
  })}</div>;
}
