...
import type { DsaPracticeMode } from "@/lib/dsa/practice-attempt";
...
function QuestionTitle({ question, applicationId, companySlug, practiceMode }: { question: DSAInterviewQuestion; applicationId?: string; companySlug?: string; practiceMode?: DsaPracticeMode }) {
  const params = new URLSearchParams();
  if (companySlug) params.set("company", companySlug);
  if (applicationId) params.set("application", applicationId);
  if (practiceMode) params.set("mode", practiceMode);
...
export function QuestionTable({ questions, companies, fixedCompanySlug, selectedCompanySlug, onToggleTopic, progress = {}, signedIn = false, accountPlatformAvailable, applicationId, practiceMode, hideTaxonomyLabels = false }: { questions: DSAInterviewQuestion[]; companies: DSACompany[]; fixedCompanySlug?: string; selectedCompanySlug?: string; onToggleTopic: (topic: string) => void; progress?: DsaProgressMap; signedIn?: boolean; accountPlatformAvailable: boolean; applicationId?: string; practiceMode?: DsaPracticeMode; hideTaxonomyLabels?: boolean }) {
  const companyMap = new Map(companies.map((company) => [company.slug, company]));
  return <div className="dsa-browser-table-wrap"><table className="dsa-browser-table"><thead><tr><th scope="col">Question</th><th scope="col">Progress</th><th scope="col">Company</th><th scope="col">Difficulty</th>{!hideTaxonomyLabels && <th scope="col">Topics</th>}<th scope="col">Source</th></tr></thead><tbody>{questions.map((question) => { const row = progress[question.id]; const status = row?.status ?? "not_started"; return <tr key={question.id}>
    <th scope="row" data-label="Question"><QuestionTitle question={question} applicationId={applicationId} companySlug={fixedCompanySlug ?? selectedCompanySlug} practiceMode={practiceMode} />{question.isSample && <small>Demo company tag</small>}</th>
    ...
    {!hideTaxonomyLabels && <td data-label="Topics"><TopicCell topics={question.topics} onToggleTopic={onToggleTopic} /></td>}
