...
import { behavioralContentStatusLabel, storyReadiness } from "@/lib/behavioral/readiness";
...
export function AnswerForm({ action, questionId, stories, applications, defaultCompany, defaultApplication, defaultStory, answer }: { action: AnswerAction; questionId: string; stories: AnswerStoryOption[]; applications: AnswerApplicationOption[]; defaultCompany?: string | null; defaultApplication?: string | null; defaultStory?: string | null; answer?: BehavioralAnswer }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (submissionPending.current || pending) return;
    submissionPending.current = true;
    const formData = new FormData(event.currentTarget);
    startTransition(() => formAction(formData));
  };
  const liveStatus = pending ? "idle" : state.status;
  return <form action={formAction} onSubmit={submit} className="behavioral-form form-shell" aria-busy={pending}>
    {answer && <input type="hidden" name={BEHAVIORAL_ANSWER_EXPECTED_REVISION_FIELD} value={answer.updated_at} />}
    <input type="hidden" name={BEHAVIORAL_ANSWER_PRIMARY_PRESENT_FIELD} value="true" />
    <input type="hidden" name={BEHAVIORAL_ANSWER_FACT_CONFIRMATION_PRESENT_FIELD} value="true" />
    <div className="behavioral-form-section"><div><h2>Prepare this question</h2><p>Choose reusable evidence and save only the framing you need. Company and application context are optional.</p></div><div className="form-grid">
    ...
    <div className="form-group"><label htmlFor="answer-status">Content state</label><select id="answer-status" name="status" defaultValue={answer?.status ?? "Draft"}>{ANSWER_STATUSES.map((status) => <option key={status} value={status}>{behavioralContentStatusLabel(status)}</option>)}</select></div>
    <div className="form-group"><label htmlFor="answer-company">Company <span>Optional</span></label><input key={company} id="answer-company" name="company_slug" list="answer-companies" maxLength={200} defaultValue={company} readOnly={Boolean(application)} aria-invalid={Boolean(state.fieldErrors?.company_slug)} aria-describedby={state.fieldErrors?.company_slug ? "answer-company-help company_slug-error" : "answer-company-help"} /><small id="answer-company-help">{application ? "Set by the selected application." : "Use this for company-specific prep without an application."}</small><BehavioralFieldError state={state} name="company_slug" /><datalist id="answer-companies">{TRACKER_COMPANIES.map((name) => <option key={name} value={name} />)}</datalist></div>
  ...
  </div></div>{(pending || state.message) && <p className={liveStatus === "error" ? "form-error" : "behavioral-save-status"} role={liveStatus === "error" ? "alert" : "status"} aria-live={liveStatus === "error" ? "assertive" : "polite"} aria-atomic="true">{pending ? "Saving answer…" : state.message}{!pending && state.conflict && <><br /><Link href={`/behavioral/questions/${questionId}`} target="_blank" rel="noopener noreferrer">Review latest answer in a new tab</Link></>}</p>}<div className="tracker-form-actions"><button className="button" type="submit" aria-disabled={pending}>{pending ? <><LoaderCircle className="spin" size={16} />Saving…</> : answer ? "Save answer" : "Create answer"}</button><Link className="button button-secondary" href={`/behavioral/questions/${questionId}`}>Cancel</Link></div></form>;
}
