...
export function StoryForm({ action, story, themes = [] }: { action: StoryAction; story?: BehavioralStory; themes?: string[] }) {
  ...
  return <form action={formAction} className="behavioral-form form-shell" onChangeCapture={() => setDirty(true)}>
    <div className="behavioral-form-section"><div><h2>Name the moment</h2><p>Use a title you will recognize quickly during preparation.</p>{story && <p className="behavioral-readiness-note"><strong>{storyReadiness(story)}</strong> based on the detail currently in Situation, Task, Action, and Result.</p>}</div><div className="form-grid">
      ...
      <div className="form-group"><label htmlFor="story-context">Company or context</label><input id="story-context" name="company_or_context" maxLength={200} defaultValue={story?.company_or_context ?? ""} /></div>
      <div className="form-group"><label htmlFor="story-role">Your role</label><input id="story-role" name="role" maxLength={160} defaultValue={story?.role ?? ""} /></div>
      <div className="form-group"><label htmlFor="story-period">Approximate period</label><input id="story-period" name="approximate_period" maxLength={100} placeholder="Q2 2025" defaultValue={story?.approximate_period ?? ""} /></div>
      <div className="form-group"><label htmlFor="story-project">Project</label><input id="story-project" name="project" maxLength={200} defaultValue={story?.project ?? ""} /></div>
    </div></div>
    ...
    <details className="tracker-form-optional" open={Boolean(story?.short_summary || story?.notes)}><summary>Short version and private notes <span>Optional</span></summary><div className="form-grid"><div className="form-group full"><label htmlFor="story-summary">60-second summary</label><textarea id="story-summary" name="short_summary" rows={4} maxLength={5000} defaultValue={story?.short_summary ?? ""} /></div><div className="form-group full"><label htmlFor="story-notes">Private notes</label><textarea id="story-notes" name="notes" rows={5} maxLength={50000} defaultValue={story?.notes ?? ""} /></div></div></details>
    {state.message && <p className="form-error" role="alert">{state.message}</p>}
    <div className="tracker-form-actions"><button className="button" type="submit" disabled={pending}>{pending ? <><LoaderCircle className="spin" size={16} />Saving…</> : story ? "Save story" : "Create story"}</button><Link className="button button-secondary" href={cancelHref} onClick={(event) => { if (dirty && !window.confirm("Discard your unsaved changes?")) event.preventDefault(); }}>Cancel</Link>{dirty && <span className="behavioral-unsaved" role="status">Unsaved changes</span>}</div>
  </form>;
}
