...
export function ProfileForm({ profile, mode, next, userId }: { profile: Profile; mode: "onboarding" | "settings"; next?: string; userId: string }) {
  ...
  return <form action={action} className="profile-form form-shell">
    ...
    <div className="profile-form-actions"><button className="button" disabled={pending} type="submit">{pending ? <><LoaderCircle className="spin" size={16} />Saving…</> : mode === "onboarding" ? "Complete setup" : "Save profile"}</button>{mode === "settings" && state.username && <a className="button button-secondary" href={`/u/${state.username}`}>View profile</a>}</div>
  </form>;
}
