...
export function ResourceDirectory() {
  ...
  <div className="resource-grid">{filtered.map((resource) => <a className="resource-card" href={resource.url} target="_blank" rel="noreferrer" key={resource.id} onClick={() => track("resource_clicked", { resource_id: resource.id, category: resource.category })}><div className="resource-meta"><StatusPill tone="accent">Demo</StatusPill><span className="resource-link"><ExternalLink size={16} /></span></div><h3>{resource.title}</h3><p>{resource.description}</p><div className="tag-list"><span className="tag">{resource.category}</span><span className="tag">{resource.type}</span><span className="tag">{resource.access}</span></div></a>)}{!filtered.length && <div className="empty-state"><strong>No resources found</strong><p>Try a broader query or reset the filters.</p></div>}</div></>;
}
