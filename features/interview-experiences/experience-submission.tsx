...
export function ExperienceSubmission({ accountPlatformAvailable, signedIn, owned }: { accountPlatformAvailable: boolean; signedIn: boolean; owned: readonly OwnedExperience[] }) {
  ...
  const edit = (item: OwnedExperience) => {
    if (!editableStatuses.has(item.status)) return;
    const round = item.interview_experience_rounds?.[0];
    setInput({ id: item.id, companyName: item.company_name, roleTitle: item.role_title, roleLevel: item.role_level ?? "", region: item.region ?? "", interviewDate: item.interview_date?.slice(0, 7) ?? "", summary: item.summary, preparationLessons: item.preparation_lessons ?? "", publicIdentity: item.public_identity, publicationConsent: item.publication_consent, roundType: round?.round_type ?? "", topics: round?.topic_labels ?? [] });
    setView("form"); setMessage(`Editing your ${item.status.replaceAll("_", " ")} report.`);
  ...
  return <div className="experience-submission" id="contribute">
    ...
    {view === "preview" ? <section className="experience-preview" aria-labelledby="experience-preview-heading"><div><h3 id="experience-preview-heading">Preview your report</h3><p>This is not public yet. It shows the bounded report that will be sent for moderation when you confirm submission.</p></div><dl><div><dt>Company</dt><dd>{preview.companyName || "Not provided"}</dd></div><div><dt>Role</dt><dd>{preview.roleTitle || "Not provided"}</dd></div><div><dt>Level</dt><dd>{preview.roleLevel || "Not provided"}</dd></div><div><dt>Region</dt><dd>{preview.region || "Not provided"}</dd></div><div><dt>Process stage</dt><dd>{preview.roundType || "Not provided"}</dd></div></dl><h4>High-level process summary</h4><p>{preview.summary || "Not provided"}</p>{preview.preparationLessons && <><h4>Preparation lessons</h4><p>{preview.preparationLessons}</p></>}<div className="experience-generate-row"><button className="button button-secondary" type="button" onClick={() => setView("form")}>Return to edit</button><button className="button" type="button" disabled={pending || !preview.publicationConsent} onClick={() => save(true)}><Send size={15} />Submit for review</button><span>{preview.publicationConsent ? "Publication consent confirmed. The report remains private until approval." : "Confirm publication consent in edit mode before submitting."}</span></div></section> : <><div className="experience-field-grid">
      ...
      <label className="form-group"><span>Interview date</span><input type="month" value={input.interviewDate} onChange={(event) => update("interviewDate", event.target.value)} /><small>Optional; month only helps readers judge freshness.</small></label>
    ...
    <div className="experience-generate-row"><button className="button button-secondary" type="button" disabled={pending} onClick={() => save(false)}>Save private draft</button><button className="button button-secondary" type="button" disabled={pending} onClick={cancelEdit}>{input.id ? "Cancel edit" : "Clear form"}</button><button className="button" type="button" disabled={pending} onClick={() => setView("preview")}>Preview report</button><span role="status" aria-live="polite">{pending ? "Saving…" : message || "Submission is optional; nothing is public without approval."}</span></div></>}
    {owned.length > 0 && <section className="experience-owned" aria-labelledby="your-experiences"><h2 id="your-experiences">Your submissions</h2><ul>{owned.map((item) => <li key={item.id}><div><strong>{item.company_name} · {item.role_title}</strong><span>{item.status.replaceAll("_", " ")} · updated {new Date(item.updated_at).toLocaleDateString()}</span>{item.status === "needs_changes" && item.review_note && <p>{item.review_note}</p>}</div><div>{editableStatuses.has(item.status) && <button className="button button-secondary" type="button" disabled={pending} onClick={() => edit(item)}>Edit</button>}{["draft", "submitted", "needs_changes"].includes(item.status) && <button className="button button-secondary" type="button" disabled={pending} onClick={() => manage(item.id, "withdraw")}><Undo2 size={14} />Withdraw</button>}{["draft", "withdrawn", "rejected"].includes(item.status) && <button className="button button-ghost" type="button" disabled={pending} onClick={() => manage(item.id, "delete")}><Trash2 size={14} />Delete</button>}</div></li>)}</ul></section>}
  </div>;
}
