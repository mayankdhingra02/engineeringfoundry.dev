...
assert.equal(plansForMockTrack("low-level-design").length, 59, "The LLD tab must expose the complete requested prompt set.");
const lldTarget = { track: "low-level-design", problem: plansForMockTrack("low-level-design")[58].slug, mode: "solo" };
assert.deepEqual(parseMockInterviewUrlState(queryFromHref(mockInterviewPageHref("/mock-interviews", lldTarget))), lldTarget, "LLD prompt selection must round-trip through the public URL state.");
...
for (const privateKey of ["notes", "strength", "improvement", "followUp", "rating", "elapsedSeconds", "sessionId", "startedAt", "promptExposure", "timingMode", "hintPolicy", "assistanceState", "sessionOutcome", "sessionIssue"]) assert.ok(!shareHref.includes(privateKey), `Copied Mock links must exclude ${privateKey}.`);
