...
for (const marker of ["plan(78)", "stale feedback triage returns zero rows", "exact feedback triage retry does not churn its revision", "legacy feedback triage fails safely without mutation", "archive requires a private rationale", "archived report disappears from public reads immediately", "approved username attribution resolves only from the contributor current public profile"]) assert.ok(feedbackPgTap.includes(marker), `feedback pgTAP evidence is missing ${marker}`);
...
assert.ok(exporter.includes('EXPORT_VERSION = "1.6"') && exporter.includes('collectAccountExportRows("feedback_submissions"') && exporter.includes('feedback: { submissions: feedbackSubmissions }'), "account export does not include only owned feedback under the bumped schema");
