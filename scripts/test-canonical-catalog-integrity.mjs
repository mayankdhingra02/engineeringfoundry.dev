...
const sdRequiredClosureMigration = read("202609040017_publish_system_design_required_closure.sql");
...
const sdRequiredClosureSeeded = seededArray(sdRequiredClosureMigration, "insert into public.system_design_item_catalog");
...
check("System Design Required-closure seed was located in its migration", sdRequiredClosureSeeded !== null);
...
if (sdConceptsSeeded) {
  const databaseConceptIds = new Set([...sdConceptsSeeded, ...(sdProductionEngineeringSeeded ?? []), ...(sdCommonPatternsSeeded ?? []), ...(sdRequiredClosureSeeded ?? [])]);
