...
const requiredClosureMigration = readFileSync(new URL("../supabase/migrations/202609040017_publish_system_design_required_closure.sql", import.meta.url), "utf8");
...
check(canonicalSystemDesignConceptIds.size === 188, "published concept catalog stays canonical");
check(canonicalSystemDesignProblemIds.size === 27, "published problem catalog stays canonical");
check(canonicalSystemDesignItemIds.size === 215, "namespaced combined catalog preserves shared concept/problem IDs");
...
for (const id of ["observability", "logs", "metrics", "distributed-tracing", "request-ids", "alerts", "slis", "slos", "error-budgets", "authn-authz", "sessions-tokens", "jwt", "oauth-oidc", "tls", "encryption", "secrets-management", "api-abuse-ddos", "tenant-authorization"]) {
...
for (const id of ["schema-data-migration", "incident-recovery-postmortems", "security-threat-modeling", "cost-efficiency", "operational-ownership", "backfill-rebuild", "control-plane-data-plane", "payments-ledgers", "distributed-file-systems", "storage-compute-separation"]) {
  check(requiredClosureMigration.includes(`'${id}'`), `Required-closure migration publishes canonical concept ${id}`);
}
check(requiredClosureMigration.includes("on conflict (id, item_type) do nothing"), "Required-closure catalog publication is idempotent");
...
check((queries.match(/if \(actorState\.state !== "authenticated"\) return \{ accountPlatformAvailable: actorState\.state === "anonymous", signedIn: false as const/g) ?? []).length === 3, "public queries preserve verified anonymous state and suppress sign-in prompts when actor verification is unavailable");
check((queries.match(/accountPlatformAvailable,/g) ?? []).length >= 6 && (queries.match(/accountPlatformAvailable: actorState\.state === "anonymous"/g) ?? []).length === 3 && queries.includes("signedIn: true as const"), "authenticated, anonymous, and unavailable query results preserve distinct account availability alongside public data");
...
check(problemPanel.includes("state.accountPlatformAvailable") && problemPanel.includes("Private design attempts are unavailable right now") && problemPanel.includes("Sign in to practice"), "problem practice panel renders unavailable, enabled signed-out, and authenticated attempt states");
