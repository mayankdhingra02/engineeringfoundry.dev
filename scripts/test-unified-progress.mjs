...
assert.ok(importRoute.indexOf("parsePreparationImportRequest(payload)") < importRoute.indexOf("await getAuthenticatedActorState()"), "browser import must strictly parse the untrusted snapshot before actor or RPC work");
assert.ok(importRoute.includes('actorState.state === "unavailable"') && importRoute.includes("PREPARATION_IMPORT_UNAVAILABLE_MESSAGE") && importRoute.includes("status: 503") && importRoute.includes('actorState.state === "anonymous"') && importRoute.includes("status: 401"), "browser import must distinguish unavailable actor verification from a verified anonymous session");
for (const marker of [
...
assert.match(accountContinuationQuery, /actorState\.state === "unavailable"[\s\S]*authenticated: false, queryFailed: true/, "an unavailable shared actor must remain unavailable rather than anonymous");
assert.match(accountContinuationQuery, /actorState\.state === "anonymous"[\s\S]*authenticated: false, queryFailed: false/, "a verified signed-out lookup must remain anonymous rather than unavailable");
...
assert.ok(activityAction.indexOf("if (!isAccountPlatformAvailable())") < activityAction.indexOf("await getAuthenticatedActorState()"), "activity actions must reject disabled accounts before actor resolution");
assert.ok(activityAction.includes('actorState.state === "unavailable"') && activityAction.includes('reason: "account-unavailable"') && activityAction.includes('actorState.state === "anonymous"') && activityAction.includes('reason: "unauthenticated"'), "activity actions must distinguish unavailable actor verification from verified signed-out state");
assert.ok(activityAction.includes("!input ||") && activityAction.includes("preparationTrackSet.has(input.track)") && activityAction.includes("preparationStatusSet.has(input.status)") && activityAction.includes('typeof input.itemId !== "string"') && activityAction.indexOf("preparationTrackSet.has(input.track)") < activityAction.indexOf("await getAuthenticatedActorState()") && activityAction.includes('input.track === "ml-design" || input.track === "behavioral"'), "the public activity action must reject malformed or forged track/status/item values before actor resolution or RPC dispatch");
