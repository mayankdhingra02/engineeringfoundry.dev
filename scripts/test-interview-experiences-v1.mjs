...
const ownerReady = resolveOwnedInterviewExperienceHistory({ data: [ownerRow], error: null });
expect(ownerReady.status === "ready" && ownerReady.limit === INTERVIEW_EXPERIENCE_OWNER_HISTORY_LIMIT && ownerReady.items[0].id === validId && ownerReady.items[0].interview_experience_rounds.map((round) => round.position).join(",") === "1,2", "Owner history must normalize IDs and deterministically order a valid private round snapshot.");
expect(resolveOwnedInterviewExperienceHistory({ data: [], error: null }).status === "ready", "A genuine empty owner history must stay ready rather than unavailable.");
for (const status of ["draft", "submitted", "needs_changes", "approved", "rejected", "archived", "withdrawn"]) expect(resolveOwnedInterviewExperienceHistory({ data: [{ ...ownerRow, status }], error: null }).status === "ready", `Owner history must accept the persisted ${status} lifecycle.`);
for (const result of [null, {}, { data: [] }, { error: null }, { data: [], error: undefined }, { data: [], error: { message: "private detail" } }, { data: [ownerRow], error: { message: "data must not win" } }, { data: Array(INTERVIEW_EXPERIENCE_OWNER_HISTORY_LIMIT + 1).fill(ownerRow), error: null }, { data: [{ ...ownerRow, extra: true }], error: null }, { data: [{ ...ownerRow, status: "deleted" }], error: null }, { data: [{ ...ownerRow, updated_at: "bad" }], error: null }, { data: [{ ...ownerRow, interview_experience_rounds: [ownerRound, ownerRound] }], error: null }]) {
  expect(resolveOwnedInterviewExperienceHistory(result).status === "unavailable", "Malformed, failed, over-limit, or invalid persisted owner history must be unavailable rather than false-empty.");
}
...
for (const status of ["submitted", "needs_changes"]) {
  const ready = resolveAdminInterviewExperienceQueue({ data: [{ ...adminRow, status }], error: null });
  expect(ready.status === "ready" && ready.limit === INTERVIEW_EXPERIENCE_ADMIN_QUEUE_LIMIT && ready.items[0].preparation_lessons === adminRow.preparation_lessons && ready.items[0].public_identity === "username", `Admin queue must preserve every public-facing field for ${status} moderation.`);
}
expect(resolveAdminInterviewExperienceQueue({ data: [], error: null }).status === "ready", "A genuine empty moderation queue must stay ready.");
for (const result of [null, {}, { data: [] }, { data: [], error: undefined }, { data: [adminRow], error: { message: "private detail" } }, { data: Array(INTERVIEW_EXPERIENCE_ADMIN_QUEUE_LIMIT + 1).fill(adminRow), error: null }, { data: [{ ...adminRow, preparation_lessons: 1 }], error: null }, { data: [{ ...adminRow, public_identity: "public" }], error: null }, { data: [{ ...adminRow, interview_experience_rounds: [{ ...adminRound, process_notes: "x".repeat(1501) }] }], error: null }]) {
  expect(resolveAdminInterviewExperienceQueue(result).status === "unavailable", "Malformed, failed, over-limit, or invalid persisted moderation data must be unavailable rather than a false empty queue.");
}
...
expect(page.includes("actorState") && page.includes("getOwnedInterviewExperienceHistory(actorState.actor)") && page.includes('{ status: "anonymous" as const }') && page.includes('{ status: "unavailable" as const }'), "Private history must stay behind the verified account/actor boundary and distinguish unavailable from anonymous through the strict production query helper.");
...
expect(queries.includes('.eq("author_id", actor.user.id)') && queries.includes("resolveOwnedInterviewExperienceHistory") && queries.includes("INTERVIEW_EXPERIENCE_OWNER_HISTORY_LIMIT"), "Owner history query must be explicitly owner-scoped, bounded, and strictly resolved.");
expect(queries.includes('.in("status", ["submitted", "needs_changes"])') && queries.includes("resolveAdminInterviewExperienceQueue") && queries.includes("INTERVIEW_EXPERIENCE_ADMIN_QUEUE_LIMIT"), "Moderation queue query must be lifecycle-scoped, bounded, and strictly resolved.");
...
console.log("Interview Experiences v1 passed: strict runtime parsing, private-history truth, revision-checked aggregates, draft-safe source integration, complete moderation, and controlled publication boundaries are present.");
