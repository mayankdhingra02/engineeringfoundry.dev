...
expect(migrations.length === 23 && latestMigration === "202608220002_create_preparation_track_progress.sql", "Production baseline must include the verified 23-migration P0.1/P0.2 history.");
for (const marker of [`all ${migrations.length} migrations`, latestMigration.replace(/\.sql$/, ""), "preparation_track_progress", "export_version` `1.4", "mock reviews, Interview Experience drafts", "active-plan preferences", "browser-progress import boundary"]) expect(checklist.includes(marker), `Hosted checklist must cover combined P0.1/P0.2 production qualification: ${marker}.`);
