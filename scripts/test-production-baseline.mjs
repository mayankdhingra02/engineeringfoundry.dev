...
for (const marker of ["SUPABASE_SERVICE_ROLE_KEY", "REMINDER_WORKER_SECRET", "supabase migration list", latestMigration, `all **${migrations.length}** migrations`, "https://<production-domain>/auth/callback", "point-in-time recovery", "owner/hosting-provider actions", "Interview Experience draft isolation", "preparation activity and active-plan isolation", "anonymous browser-import no-overwrite boundary", "canonical professional URLs", "unsafe legacy professional-link values", "set_interview_preparation_checklist_item", "Checklist items must be updated individually", "set_dsa_question_quick_progress", "Exactly one quick progress value is required", "NEXT_PUBLIC_ACCOUNTS_ENABLED=false"]) expect(runbook.includes(marker), `Production runbook is missing ${marker}.`);
expect(migrations.length === 29 && latestMigration === "202609030002_set_dsa_question_quick_progress.sql", "Production baseline must include all 29 reviewed migrations through atomic DSA quick-progress updates.");
for (const marker of [`all ${migrations.length} migrations`, latestMigration.replace(/\.sql$/, ""), "preparation_track_progress", "export_version` `1.5", "mock reviews, Interview Experience drafts", "active-plan preferences", "browser-progress import boundary", "feedback/admin operations", "canonical GitHub and LinkedIn URLs", "unsafe legacy professional-link values", "set_interview_preparation_checklist_item", "SQLSTATE `0A000`", "set_dsa_question_quick_progress", "Exactly one quick progress value is required"]) expect(checklist.includes(marker), `Hosted checklist must cover combined P0.1–P0.8 production qualification: ${marker}.`);
