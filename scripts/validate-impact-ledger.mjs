...
function validateSnapshot(record, path) {
  ...
  required(record.measurement_window, "start", path); required(record.measurement_window, "end", path); required(record, "source_reference", path);
...
if (errors.length) {
  console.error(`Impact-ledger validation failed:\n- ${errors.join("\n- ")}`);
  process.exit(1);
}
